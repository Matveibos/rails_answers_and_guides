BAGS 
     ...
     3. В админке при измениние памятника по дефолту ставиться значение USA если в поле кладбище ничего не выбранно. +
     4. В админке нет поля кладбище для памятников +
     5. точнее если кладбище не задано  +
     6. Предупреждение пропадает на страницах, к которым ты не имеешь доступа, после добавления памятника
     7. Команда monumentBelongsUser  вызывается сотни раз
...
12. Set default values for chosen
                 ...
                 <select
                  ...
                  advancedFilterCategorySelectedArray = (selected_category) ->
                    selected_category_index_array = []
                    category_name = []
